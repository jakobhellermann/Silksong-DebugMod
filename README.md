...
2. clone this repo ([generate from this template](https://github.com/new?template_name=NineSols-DebugMod&template_owner=jakobhellermann), then update the `.csproj`
...
dotnet publish
```
it will build the DLL of your mod (`Source/bin/Release/netstandard2.1/publish/DebugMod.dll`), then use `tcli` to
...
Go into the `DebugMod.csproj` file and fill out the `<ProfileDir>` and uncomment the `<CopyDir>` below it.
